...
const CustomToolbar = ({ onButtonClick, selectedRows }) => {
  ...
  const { handleDelete, handleClose, open, alertMessage, severity } = useDeleteHook('InsuranceProvider/DeleteInsuranceProvider'); 
...
function createData(id, provider, email, dateCreated, dateModified) {
  return {id, provider, email, dateCreated, dateModified };
};
...
const columns = [ 
  {
    ...
    renderCell: (params) => (
      <CustomButton
        ...
        destination={`/editInsuranceProvider/${params.row.id}`}
        color="warning"
        variant="contained"
        size="small"
      />
...
const InsuranceProvider = () => {
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          let apiUrl = `${API_URL}api/InsuranceProvider/ShowInsuranceProvider`;
  ...
  return (
    ...
    <div className="content"> 
        ...
        <div className="admin-dashboard-text-div pt-5"> 
            <h1 className="h1-dashboard">Insurance Provider</h1>
        </div>
