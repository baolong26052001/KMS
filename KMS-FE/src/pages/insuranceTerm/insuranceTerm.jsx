...
const CustomToolbar = ({ onButtonClick, selectedRows }) => {
  ...
  const { handleDelete, handleClose, open, alertMessage, severity } = useDeleteHook('Term/DeleteTerm'); 
...
function createData(id, content, dateCreated, dateModified) {
  return {id, content, dateCreated, dateModified };
};
...
const columns = [ 
  {
    ...
    renderCell: (params) => (
      <CustomButton
        ...
        destination={`/editInsuranceTerm/${params.row.id}`}
        color="warning"
        variant="contained"
        size="small"
      />
...
const InsuranceTerm = () => {
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          let apiUrl = `${API_URL}api/Term/ShowTerm`;
  ...
  return (
    ...
    <div className="content"> 
        ...
        <div className="admin-dashboard-text-div pt-5"> 
            <h1 className="h1-dashboard">Insurance Term</h1>
        </div>
