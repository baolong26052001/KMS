...
function createData(transactionDate, id, memberId, fullName, contractId, loanId, loanTerm, debt, totalDebtMustPay, payback) {
  return { transactionDate, id, memberId, fullName, contractId, loanId, loanTerm, debt, totalDebtMustPay, payback };
}
...
const columns = [
  ...
  { field: 'debt', headerName: 'Debt', minWidth: 100, flex: 1, },
  {
...
const Loanstatement = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let apiUrl = `${API_URL}api/LoanStatement/ShowLoanStatement`;
        let searchApi = ``;
  
        if (startDate || endDate) {
          apiUrl = `${API_URL}api/LoanStatement/FilterLoanStatement?startDate=${encodeURIComponent(dayjs(startDate).format('YYYY/MM/DD'))}&endDate=${encodeURIComponent(dayjs(endDate).format('YYYY/MM/DD'))}`;
          if (searchTerm) {
            searchApi = `${API_URL}api/LoanStatement/SearchLoanStatement?searchQuery=${encodeURIComponent(searchTerm)}`;
          }
        } else if (searchTerm) {
          apiUrl = `${API_URL}api/LoanStatement/SearchLoanStatement?searchQuery=${encodeURIComponent(searchTerm)}`;
        }
        ...
        if (Array.isArray(apiResponseData)) {
          ...
          const updatedRows = filteredRows.map(row =>
            createData(row.transactionDate, row.id, row.memberId, row.fullName, row.contractId, row.loanId, row.loanTerm, row.debt, row.totalDebtMustPay, row.payback)
          );
